...
from ctypes import (cdll,
                    c_byte, c_double, c_float, c_int, c_short,
                    c_ubyte, c_uint, c_ulong, c_ulonglong, c_ushort,
...
def minSigned(x): return -2**x/2


def maxSigned(x): return (2**x/2)-1


def maxUnsigned(x): return (2**x)-1
...
class c_py3_unicode_p(c_char_p):
    ...
    def from_param(cls, val):
        return c_char_p.from_param(val.encode(encoding))


c_stringy = c_py3_unicode_p
